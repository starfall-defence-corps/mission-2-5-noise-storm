...
MISSION="Mission 2.5: Noise Storm"
...
# A dead noise storm makes verification INCONCLUSIVE — never a pass. If you see
# this after 'make setup', reset the lab so the storm resumes.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(dirname "$SCRIPT_DIR")"
STATUS_FILE="$ROOT_DIR/.lab/noise/status.json"
if docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^sdc-noise$'; then
    if [ -f "$STATUS_FILE" ]; then
        NOW=$(date +%s)
        TS=$(python3 -c "import json,sys; print(json.load(open('$STATUS_FILE')).get('updated_ts',0))" 2>/dev/null || echo 0)
        AGE=$(( NOW - TS ))
        if [ "$TS" -gt 0 ] && [ "$AGE" -lt 30 ]; then
            pass "Noise storm active (last signal ${AGE}s ago)"
        else
            warn "Noise storm signal is stale (${AGE}s old)" "The range may have stalled — run 'make reset' before 'make test' so results are conclusive"
        fi
    else
        warn "Noise storm has not published a signal yet" "Give 'make setup' a few more seconds, or run 'make reset'"
    fi
fi
